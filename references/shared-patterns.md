...
Projects that use workspace mode declare modules in the root `CLAUDE.md`. This table is auto-generated by `/design:init` workspace setup or can be maintained manually. See the **Workspace Detection** pattern below for how this table is used during module discovery.
...
When both `.gitmodules` and a `### Workspace Modules` table exist, `.gitmodules` takes precedence. The `### Workspace Modules` table is treated as a cache that `/design:init` generates from `.gitmodules`. If they diverge, skills SHOULD use `.gitmodules` and suggest re-running `/design:init` to update the table.
