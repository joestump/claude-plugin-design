...
- With `--review`, spawns a drafter + architect team (max 2 revision rounds)
- **After writing the spec**, offers to plan a sprint:
  - Detects available issue trackers ([Beads](https://github.com/steveyegge/beads),
    GitHub via MCP or `gh` CLI, Gitea via MCP)
  ...
  - Falls back to a structured markdown plan if no tracker is available
