...
After writing the spec, the agent offers to break it down into trackable
work items — no manual issue creation needed. The agent handles sprint
...
1. **Detects available issue trackers**: The agent checks for
   ...
   `gh` CLI), and Gitea (via MCP) — and asks you to choose if multiple
   are available. If none are found, it outputs a structured markdown
   plan instead.

2. **Breaks down requirements into issues**: Each `### Requirement:`
   in the spec becomes a task, grouped under an epic for the
   specification. Complex requirements with multiple scenarios get
   sub-tasks.

3. **Writes acceptance criteria**: Every issue includes criteria
...
4. **Sets up dependencies**: For Beads, the agent uses `bd dep add` to
   link tasks that block each other based on requirement relationships.

5. **Reports the plan**: A summary of epics, tasks, and sub-tasks
   created, with links to where they live.

:::info No human scoping required
Claude Teams excel at sprint planning. The agent reads the spec,
understands the requirement relationships, and creates well-structured
issues with proper acceptance criteria. You review the plan, not author it.
:::
...
After the spec is written, the agent asks:

> *"Want me to plan a sprint from this spec? I'll break requirements
> into issues with acceptance criteria."*

### 3. Agent plans the sprint

You say yes, and the agent:

- **Detects Beads** (finds `.beads/` in the project root)
...
You can then do the same for a second spec:

```
/design:spec WebSocket Gateway
```

The agent writes SPEC-0005 and again offers to plan — creating a second
epic with its own tasks, acceptance criteria, and dependencies.
...
If no tracker is detected, the agent outputs a structured markdown sprint
plan that you can use to create issues manually.
