...
2. **Saves your tracker preference**: On first run, the agent offers to
   save your chosen tracker to `.claude-plugin-design.json` so you're not asked again.
...
6. **Creates project groupings**: For each epic, creates a tracker-native
   project (GitHub Projects V2, Gitea projects, GitLab milestones) and
   adds the epic and its tasks. Use `--project "Sprint Name"` for a
   single combined project, or `--no-projects` to skip.

7. **Adds branch conventions**: Each issue body gets a `### Branch`
   section with the suggested branch name (`feature/{issue-number}-{slug}`
   for tasks, `epic/{issue-number}-{slug}` for epics). Skip with
   `--no-branches`.

8. **Adds PR conventions**: Each issue body gets a `### PR Convention`
   section with the tracker-specific close keyword (e.g., `Closes #42`
   for GitHub). Pairs with branch conventions (`--no-branches` disables
   both).

9. **Reports the plan**: A summary of epics, tasks, and sub-tasks
...
- **Creates a project** "Notification Service" in GitHub Projects V2 and adds all issues
- **Adds branch names** to each issue: `feature/42-event-type-registry`, `feature/43-delivery-rules-engine`, etc.
- **Adds PR conventions**: each issue body includes `Closes #42` referencing itself and `Part of #41` referencing the epic
