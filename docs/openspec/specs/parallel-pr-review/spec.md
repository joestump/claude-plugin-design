...
The skill SHALL detect the user's tracker using the same detection flow as `/design:plan` (SPEC-0007, Requirement: Tracker Detection). The skill MUST check `.design.json` for a saved tracker preference before running detection.
...
- **WHEN** `.design.json` contains `review.merge_strategy`
...
The skill SHALL read review configuration from `.design.json`. The skill SHALL NOT write to `.design.json` (it is a consumer, not a producer of configuration).

#### Scenario: Review config schema

- **WHEN** `.design.json` contains a `review` section
...
- **WHEN** `.design.json` does not contain a `review` section
