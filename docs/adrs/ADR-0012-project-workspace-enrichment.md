...
Projects receive an iteration field named "Sprint" with a default cycle length of 2 weeks. Stories are assigned to iterations based on their dependency ordering: foundation stories go into Sprint 1, dependent stories into Sprint 2, and so on. The cycle length is configurable via `.design.json`:
...
The following keys are added to the `.design.json` schema (all optional, backward-compatible):
...
* Bad, because `.design.json` gains three new keys under `projects`, further expanding its configuration surface area
...
9. `.design.json` `projects.iteration_weeks` is respected when creating iteration fields
10. `.design.json` `projects.views` is respected when creating named views
11. `.design.json` `projects.columns` is respected when creating Gitea board columns
...
- `.design.json` additions are fully backward-compatible. Existing `.design.json` files without the new keys will use the defaults specified in this ADR.
