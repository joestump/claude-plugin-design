...
- MUST drop the Status column entirely when zero artifacts in the corpus have a parseable status — a uniformly-blank column reads as "all unknown" and is worse than no column at all. When *some* artifacts have status and others do not, render missing as `—` so the asymmetry is visible
